...
public class CFEDFUPkgRsrcRetrievalDAO extends SCPODAO {
	...
	private static final int INDEX_RSRC_STARTDATE = 1;//1
	private static final int INDEX_PKG = 2;//2
	private static final int INDEX_PKG_STARTDATE = 3;//3
	private static final int INDEX_DFU = 4;//4
	private static final int INDEX_USAGE = 5;//5
	private static final int INDEX_AUX_USAGE = 6;//6
	private static final String nullString = "nullString";//6
	protected String getLoadForBatchStmt()
    {
        StringBuffer query = new StringBuffer(" SELECT CNPO.RSRC_STARTDATE, CNPO.PKG_ID, CNPO.PKG_STARTDATE, ");
        query.append(" CNPO.NPO_DFU_ID, CNPO.USAGE, CNPO.AUX_USAGE ");
    ...
    public NPOOptimizeData loadForBatch(String resrcId) throws ManuDBException
    {
        ...
        try
        {
            ...
            return loadOptimizeDataSet(rs , resrcId);
    ...
    private NPOOptimizeData loadOptimizeDataSet(ResultSet rs , String resrcId) throws SQLException
    {
        Map dataSet = new HashMap();
        java.sql.Date startDate = null;
        String rsrcId = resrcId;
        String pkgId = null;
        ...
        String rsrc = null, oldRsrc ;
        String pkg = null, oldPkg ;
        String dfu = null, oldDFU ;
        ...
        rs.next();
		oldRsrc = rsrcId + ":" + rs.getDate(INDEX_RSRC_STARTDATE);
		oldPkg = rs.getString(INDEX_PKG) + ":" + rs.getDate(INDEX_PKG_STARTDATE);
		oldDFU = rs.getString(INDEX_DFU) + ":" + rs.getDate(INDEX_PKG_STARTDATE);
		do
		{
            ...
            if ((!oldDFU.equals(dfu)))//set old as current for first time..!!
            {
